...
class AzureVMManager:
    ...
    def _update_state(self, **kwargs):
        """Update current state with new values and save immediately"""
        if self.state:
            ...
            self.state = VMState(
                vm_name=kwargs.get('vm_name', names['vm_name']),
                resource_group=kwargs.get('resource_group', names['resource_group']),
                disk_name=kwargs.get('disk_name', names['disk_name']),
                disk_resource_id=kwargs.get('disk_resource_id', ''),
                network_interface_id=kwargs.get('network_interface_id', ''),
                public_ip_id=kwargs.get('public_ip_id', ''),
                created_at=kwargs.get('created_at', datetime.now().isoformat()),
                vm_resource_id=kwargs.get('vm_resource_id', None),
                vm_exists=kwargs.get('vm_exists', False)
            )
            self._save_state(self.state)
    ...
    def _install_ssh_server(self, rg_name: str, vm_name: str):
        ...
        try:
            ...
            # Create execution command with parameters
            # Convert Python booleans to strings for PowerShell command line
            install_wsl_str = "true" if self.config.install_wsl else "false"
            reboot_after_wsl_str = "true" if self.config.reboot_after_wsl else "false"

            execution_command = f"powershell.exe -ExecutionPolicy Unrestricted -File {script_name} -PublicKey \"{public_key}\" -AdminUser \"{self.config.admin_username}\" -InstallWSL \"{install_wsl_str}\" -RebootAfterWSL \"{reboot_after_wsl_str}\""
            ...
            self.logger.info(f"   - InstallWSL: {self.config.install_wsl} → '{install_wsl_str}'")
            self.logger.info(f"   - RebootAfterWSL: {self.config.reboot_after_wsl} → '{reboot_after_wsl_str}'")
    ...
    def create_vm(self) -> VMState:
        ...
        # Save initial state after resource group creation
        names = self._get_resource_names()
        self._update_state(
            resource_group=rg_name,
            created_at=datetime.now().isoformat()
        )
        self.logger.info(f"💾 State saved after resource group creation: {self.state_file}")
        ...
        # Save state after network infrastructure creation
        self._update_state(
            network_interface_id=nic_id,
            public_ip_id=pip_id
        )
        self.logger.info(f"💾 State saved after network infrastructure creation")
        ...
        # Save state after VM creation request is submitted
        # This includes the disk resource ID that will be created
        ...
        self._update_state(
            ...
            vm_exists=False  # VM is being created but not yet provisioned
        )
        self.logger.info(f"💾 State saved after VM creation request submission")
        ...
        # Save state after VM provisioning completes
        self._update_state(
            vm_resource_id=vm_result.id,
            vm_exists=True
        )
        self.logger.info(f"💾 State saved after VM provisioning completion")
        ...
        # State is already saved incrementally during creation
        # Final state update after SSH configuration completes
        self.logger.info(f"💾 Final state saved at: {self.state_file}")
    ...
    def destroy_vm_keep_disk(self) -> bool:
        ...
        try:
            ...
            # Update state
            self._update_state(vm_exists=False, vm_resource_id=None)
    ...
    def recreate_vm_with_disk(self) -> VMState:
        ...
        # Check for essential state information
        if not self.state.resource_group or not self.state.disk_name:
            raise ValueError("Incomplete state: missing resource group or disk name. Cannot recreate VM.")
        ...
        try:
            ...
            # Update disk resource ID if it was missing
            if not self.state.disk_resource_id:
                self._update_state(disk_resource_id=disk.id)
        except Exception as e:
            raise ValueError(f"Disk {self.state.disk_name} not found: {e}")

        # Verify network interface exists or recreate if missing
        nic_id = self.state.network_interface_id
        pip_id = self.state.public_ip_id

        try:
            if not nic_id:
                raise Exception("Network interface ID missing from state")
            nic = self.network_client.network_interfaces.get(
                self.state.resource_group, names['nic_name']
            )
            self.logger.info(f"Using existing network interface: {nic.id}")
        except Exception as e:
            self.logger.warning(f"Network interface not found or invalid, recreating: {e}")
            nic_id, pip_id, _ = self.create_network_infrastructure(self.state.resource_group)
            # Update state with new network resources
            self._update_state(network_interface_id=nic_id, public_ip_id=pip_id)
        ...
        # Update state
        self._update_state(vm_resource_id=vm_result.id, vm_exists=True)
